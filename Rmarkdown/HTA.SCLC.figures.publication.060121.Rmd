...
hta_dir = '~/Documents_NotToICloud/MSKCC/Research/HTA/publication.010920/' #YOU WILL NEED TO CHANGE THIS PATH
idir = paste0(hta_dir,'out.SCLC.010920/')
...
main_dir = '/Users/chanj3/Documents_NotToICloud/MSKCC/Research/HTA/publication.010920/out.SCLC.010920/MAST/MAST.individual/'

df_all = NULL
for (sample in names(clust_interest)) {
  deg_fn = paste0(main_dir, sample, sprintf('/MAST.clusters_phenographLV/clusters_phenographLV.%d.csv', clust_interest[sample]))
...
### Can be downloaded at https://data.humantumoratlas.org/
ref_dir = '~/Documents_NotToICloud/MSKCC/Research/HTA/MIBI/ref/TMA_annotations/'
patient_df = read.csv(paste0(ref_dir, 'TMA_sample_annotations.with_OS_PLCG2.final.csv'), sep = ',', row.names=1)

patient_df = patient_df %>% dplyr::select(OS.from.tissue.collection, Recurrence.censor..0...not.recurred.1...recurred., Vital.Status..0.Alive..1.Dead., Histology, Tissue.Stage, Site.of.tissue, Treated., ASCL1, NEUROD1)

colnames(patient_df) = c('time', 'Recurrence_censor', 'status','Histology','Stage','Tissue','Treated','ASCL1','NEUROD1')
...
plot_df$RRBP1_percent = log(feature_df[rownames(plot_df), 'RRBP1....tumor'])

plot_df$NEUROD1 = factor(plot_df$NEUROD1)
plot_df$NEUROD1_percent = factor(feature_df[rownames(plot_df), 'NEUROD1'])

plot_df$Treatment = factor(gsub('yes','Y', gsub('no','N',plot_df$Treated) ))
plot_df$Tissue[grepl('lung|upper|lower|middle',plot_df$Tissue)] = 'N'
plot_df$Tissue[plot_df$Tissue != 'N'] = 'Y'
plot_df$Tissue = factor(plot_df$Tissue)
plot_df$Combined_Histology = factor(as.numeric(grepl('/',plot_df$Histology)))
...
```{r}
### MIBI PLCG2+ as a continuous target variable without adjustment
plot_df2 = plot_df %>% dplyr::rename(Distant_Metastasis=Tissue ) #Variant_Subtype=Subtype, 
...
### Can be downloaded at https://data.humantumoratlas.org/
ref_dir = '~/Documents_NotToICloud/MSKCC/Research/HTA/publication.010920/out.SCLC.010920/survival/'
...
### Can be downloaded at https://data.humantumoratlas.org/
cell_df = read.table('~/Documents_NotToICloud/MSKCC/Research/HTA/SCLC.010920/out.SCLC.SCLCtype_deg.SCLC_only.010920/obs.SCLC.SCLCtype_deg.SCLC_only.010920.txt', sep ='\t', header = T)
tmp = cell_df %>% dplyr::select(batch, treatment_mast)
tmp$batch = gsub('RU1108.*', 'RU1108a', tmp$batch)
tmp = tmp[!duplicated(tmp$batch),]

tmp$treatment_mast = gsub('.*IO.*', 'ChemoIO_1L', tmp$treatment_mast)
tmp$treatment_mast = gsub('.*TMZ.*', 'Chemo_1L', tmp$treatment_mast)

rownames(tmp) = tmp$batch

patient_df$Treatment_Type = tmp[patient_df$Lab.ID,'treatment_mast']
...
deg_dir = '/Users/chanj3/Documents_NotToICloud/MSKCC/Research/HTA/publication.010920/out.SCLC.010920/MAST/MAST.SCLC_subtype/'
...
genes_interest = readLines(paste0(hta_dir,'out.SCLC.010920/deg.SCLCtype.interest.txt'))
